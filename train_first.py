...
def main(config_path):
    ...
    # load data
    val_list = get_data_path_list(train_path)
            ...
            try:
                ...
                if epoch >= TMA_epoch:
                    ...
                    if torch.isnan(d_loss):
                        ...
                        accelerator.clip_grad_norm_([p for k in ['msd', 'mpd'] for p in model[k].parameters()],
                                                    max_norm=1.0)
                ...
                if torch.isnan(g_loss):
                    ...
                    accelerator.clip_grad_norm_(
                        [p for k in ['text_encoder', 'style_encoder', 'decoder'] for p in model[k].parameters()],
                        max_norm=1.0)
                    if epoch >= TMA_epoch:
                        accelerator.clip_grad_norm_(
                            [p for k in ['text_aligner', 'pitch_extractor'] for p in model[k].parameters()], max_norm=1.0)
